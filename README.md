...
The `/tokens` command also displays your MiniMax coding plan balance when configured:
...
# Add your MiniMax API key (get from platform.minimax.io)
nano .env
```

Set `MINIMAX_API_KEY` in `.env`. The file is gitignored for security.

**Output includes:**
- Usage percentage (used/remaining)
- Time window (e.g., 05:00-10:00 UTC)
- Reset countdown
...
├── LATEST_CHANGES_REVIEW.md       # Comprehensive review of latest changes
